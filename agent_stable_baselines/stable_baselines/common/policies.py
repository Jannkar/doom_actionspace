...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    :param cnn_extractor: (function (TensorFlow Tensor, ``**kwargs``): (TensorFlow Tensor)) the CNN feature extraction
    :param feature_extraction: (str) The feature extraction type ("cnn" or "mlp")
    :param adapter_old_action_space: (action_space) Old action space, for which we are building an adapter for (mapping actions)
    :param kwargs: (dict) Extra keyword arguments for the nature CNN feature extraction
    """

    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", adapter_old_action_space=None, **kwargs):
        ...
        with tf.variable_scope("model", reuse=reuse):
            with tf.variable_scope("base_nn_params"):
                if feature_extraction == "cnn":
                    pi_latent = vf_latent = cnn_extractor(self.processed_obs, **kwargs)
                else:
                    pi_latent, vf_latent = mlp_extractor(tf.layers.flatten(self.processed_obs), net_arch, act_fun)

            if adapter_old_action_space is not None:
                # Build pi_latent for the older action space, 
                # select an action from there and feed that forward to 
                # new action space. This will effectively kill the gradient
                # Freezing of vf done in other functions
                self.value_fn = linear(vf_latent, 'vf', 1)

                old_pdtype = make_proba_dist_type(adapter_old_action_space)
                old_proba_distribution, _, _ = old_pdtype.proba_distribution_from_latent(pi_latent, vf_latent, init_scale=0.01)
                old_action = old_proba_distribution.sample()
                # Turn categorical actions into one-hot-encoded, for convenience
                if type(adapter_old_action_space) == Discrete:
                    pi_latent = tf.one_hot(old_action, adapter_old_action_space.n)
                elif type(adapter_old_action_space) == Box:
                    # Nothing to change here (hopefully?)
                    pi_latent = old_action
                else:
                    raise ValueError("Unhandled old distribution type %s" % str(type(adapter_old_action_space)))
                # Kill the gradient, just in case
                pi_latent = tf.stop_gradient(pi_latent)
