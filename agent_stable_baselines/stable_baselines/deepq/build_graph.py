...
def build_train(q_func, ob_space, ac_space, optimizer, sess, grad_norm_clipping=None, gamma=1.0, double_q=True,
                scope="deepq", reuse=None, param_noise=False, param_noise_filter_func=None, freeze_base_nn = False, freeze_vf = False, freeze_last_fc = False):
    """
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        all_q_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=tf.get_variable_scope().name + "/model")
        
        ### Modified for Doom experiments ###
        # TODO update for freezing adapter values etc.
        q_func_vars = []
        if freeze_base_nn or freeze_vf:
            # Parse out non-trainable variables
            print("Non-trainable params:")
            for param in all_q_vars:
                if "action_value" in param.name and freeze_base_nn and "fully_connected" not in param.name: #Freezing CNN
                    print(param.name)
                    pass
                elif "action_value" in param.name and freeze_base_nn and "fully_connected" in param.name and freeze_last_fc: #Freezing last fc if adapter
                    print(param.name)
                    pass
                elif "state_value" in param.name and freeze_vf: #Freezing state_value if freeze_vf
                    print(param.name)
                    pass
                else:
                    q_func_vars.append(param)
        else:
            q_func_vars = all_q_vars

        print("")
        print("Trainable params:")
        for p in q_func_vars:
            print(p.name)
        ...
        all_target_q_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
        ...
        if freeze_base_nn or freeze_vf:
            # Parse out non-trainable variables
            for param in all_target_q_vars:
                if "action_value" in param.name and freeze_base_nn and "fully_connected" not in param.name:
                    pass
                elif "action_value" in param.name and freeze_base_nn and "fully_connected" in param.name and freeze_last_fc: #Freezing last fc if adapter
                    pass
                elif "state_value" in param.name and freeze_vf:
                    pass
                else:
                    target_q_func_vars.append(param)
        else:
            target_q_func_vars = all_target_q_vars
