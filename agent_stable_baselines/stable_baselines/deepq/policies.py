...
class FeedForwardPolicy(DQNPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None,
                 ...
                 adapter_old_action_space=None, **kwargs):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            if adapter_old_action_space is not None:
                # Only support discrete-to-discrete adapters
                if type(adapter_old_action_space) != Discrete:
                    raise ValueError("Adapter old action space must be Discrete")
                num_actions = adapter_old_action_space.n
            ...
            #self.adapter_q_values_first = q_out #REMOVE
            
            # Add adapter layer if needed
            if adapter_old_action_space is not None:
                # Create new linear mapping between old and new Q-values
                with tf.variable_scope("adapter"):
                    q_out = tf_layers.fully_connected(q_out, num_outputs=self.n_actions, activation_fn=None)


        #self.adapter_q_values_second = q_out #REMOVE
        #self.adapter_1 = [] #REMOVE
        #self.adapter_2 = [] #REMOVE
    ...
    def step(self, obs, state=None, mask=None, deterministic=True):
        q_values, actions_proba = self.sess.run([self.q_values, self.policy_proba], {self.obs_ph: obs}) #REMOVE , adapter_q_1, adapter_q_2  # self.adapter_q_values_first, self.adapter_q_values_second

        #REMOVE
        #self.adapter_1.append(adapter_q_1)
        #adapter_1_mean = np.mean(self.adapter_1, axis=0)
        #self.adapter_2.append(adapter_q_2)
        #adapter_2_mean = np.mean(self.adapter_2, axis=0)
        #np.save("Adapter_3_q_layer_1.npy", self.adapter_1)
        #np.save("Adapter_3_q_layer_2.npy", self.adapter_2)
        #---#
